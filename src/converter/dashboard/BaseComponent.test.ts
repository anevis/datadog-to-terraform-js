...
describe('Construct Basic Component terraform.', () => {
    ...
    it('Construct Basic Component terraform option value with escaped double quotes.', () => {
        const component = new DummyBaseComponent({
            text: 'RDS',
            dummy: '@response_time by @path over \\"service:xplan @path:*\\"'
        });

        const terraformStr = component.toTerraform();

        expect(terraformStr).toBe(
            'text = "RDS" dummy = "@response_time by @path over \\\\"service:xplan @path:*\\\\""'
        );
    });
});
